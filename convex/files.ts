...
export const getFolderContents = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Sort: folders first, them files, alphabetically within each group
...
export const createFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if file with same name exists in this parent folder
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId)
...
export const createFolder = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Check if folder with same name exists in this parent folder
    const files = await ctx.db
      .query("files")
      .withIndex("by_project_parent", (q) =>
        q.eq("projectId", args.projectId).eq("parentId", args.parentId)
...
export const renameFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existing) {
      throw new Error(
        `A ${file.type} with the same name already exists in this location`
      );
    }
